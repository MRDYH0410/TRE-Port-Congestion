...
\begin{tikzpicture}[
 ...
 mod/.style={draw,rounded corners=2pt,align=center,minimum height=11mm,
 text width=29mm,fill=white},
 state/.style={draw=transgray,dashed,rounded corners=2pt,align=center,
 minimum height=11mm,text width=29mm,fill=transgray!7},
...
\node[mod,draw=projorange,fill=projorange!9,right=of risk] (scenario)
 {Module 2\\common scenario family\\dual conditional weights};
\node[mod,draw=ctrlpurple,fill=ctrlpurple!8,right=of scenario] (control)
 {Module 3\\feasible upper level control\\MPC, BC, and SAC};
\node[mod,draw=suegreen,fill=suegreen!8,below=of control] (behavior)
 {Module 4\\oldest first release\\vintage SUE and selector};
\node[mod,draw=transgray,fill=transgray!8,left=of behavior] (transition)
 {Module 5\\tagged transition\\loss and audits};
\node[state,left=of transition] (next)
 {Complete next state $s_{t+1}$\\and clearance state};
\node[state,above=of control] (offline)
 {Offline teacher and learning\\using the same nested calls};

\draw[flow] (risk) -- node[above,font=\scriptsize]{belief and lead time forecast} (scenario);
\draw[flow] (scenario) -- node[above,font=\scriptsize]{scenario support and weights} (control);
\draw[flow] (control) -- node[right,font=\scriptsize]{$a_t\in\mathcal A^{\mathrm{feas}}(s_t)$} (behavior);
\draw[flow] (behavior) -- node[above,font=\scriptsize]{$z_t^\star$ and certificate} (transition);
\draw[flow] (transition) -- node[above,font=\scriptsize]{$s_{t+1},\ell_t^{\mathrm{op}}$} (next);
\draw[call] (next.north west) to[bend left=34]
 node[above,font=\scriptsize]{released information and histories} (risk.west);
\draw[call] (next.north) to[bend left=17]
 node[below,font=\scriptsize]{closed Markov state} (control.west);
\draw[call] (offline) -- (control);
\draw[audit] (scenario.south) to[bend right=12]
 node[below,font=\scriptsize]{outer rollout paths} (transition.north);
\end{tikzpicture}
...
& Common random paths over the declared three dimensional grid and the independent optimistic absorption program
& A violated certificate is sufficient. A nonviolated cell is not a feasibility guarantee \\
...
& Prespecified one factor screening followed by selected interactions at the historical and policy switch anchors
& Conditional robustness. No high dimensional grid is described as empirical calibration \\
\bottomrule
\end{longtable}
\endgroup
...
Network exposure
& $\varrho^{Q,\mathrm{ref}}$
& Activity scale ratio in \cref{eq:network-exposure-reference}. Robustness uses one half, one, and twice the reference
...
Waiting attrition
& $\vartheta_k(j)$
& The benchmark uses $\vartheta_k(j)=(j/\bar J)^2$ with $\vartheta_k(\bar J)=1$. Concave, linear, and convex profiles are compared in robustness analysis
& Designed behavioural family
& Duration exit and waiting boundary \\
Route choice
& $\theta_k$
& The benchmark uses $\theta_k=1$ in reciprocal generalised cost units after one common cost normalisation. Low and high regimes are frozen before robustness analysis
& Partially identified behaviour
& Route concentration, waiting, and direct exit response \\
Disclosure
& $\lambda_{krt}$, $\gamma_I$, $\sigma^W_{krt}$
& $0\le\lambda_{krt}\le1$ and the benchmark registers $\gamma_I=1$. Route wise one step Passive waiting RMSE is 0.312009 weeks for Khor Fakkan, 1.290553 for Fujairah, and 0.047840 for Sohar using 40 errors on each of two held out paths. The signal follows \cref{eq:522-public-wait-signal}
& Formal action and held out forecast calibration
& Credibility, disclosure cost, and information value \\
Maritime pipeline
& $\ell^M_{kr,\tau\mid t}$, $\tau^M_{kr}$
& Each dispatch cohort kernel sums to one. Reference maritime lags are two weeks for Khor Fakkan, three for Fujairah, and four for Sohar. Alternative designs preserve route and provenance tags
& Designed timing anchored to declared route ordering
& Lag sensitivity and recurrent closure mechanism \\
Reference service
& $\bar\mu_i^B$, $\bar\mu_i^Y$, $\bar\mu_i^G$, $\bar\kappa_e$
& Gateway activity scale $s_i$ supplies the balanced reference. Bottleneck architectures perturb one declared stage or shared corridor
...
& Reported as weeks of reference service through $\bar Q_i^q/\bar\mu_i^q$. The neutral threshold equals one service week
& Declared operational normalisation
& Pressure, overload, and threshold robustness \\
Physical feedback
& $\psi_i^Y$, $\psi_i^M$, $\omega_{ie,t}$
& The reference response is $\psi(r)=[1+\pos{r-1}]^{-1}$ with unit strength and threshold. Corridor shares use two lagged weeks with unit fallback weight. Absence and stronger feedback regimes are compared
...
Resource loss
& $c_B$, $c_Y$, $c_G$, $c_M$, $c^W_{kj}$, $c^E_k$, $\Delta c^{\mathrm{res}}_{kr}$
& The benchmark uses unit queue and overflow cost, $c^W_{kj}=j+1$, $c^E_k=21$, terminal unit cost $c^E_k$, and route resource increments zero, one, and two for Khor Fakkan, Fujairah, and Sohar. Handling, trucking, and border increments are zero under the identical benchmark topology
...
& Benchmark readiness lead is eight weeks and direct procurement lead is zero. Expiry follows the persistence rules registered for the two resources
& Designed operational inputs
& Lead time value, expiry loss, and false warning cost \\
Action cost and budget
& $c^R$, $c^V$, $c^{\mathrm{ex}}$, $c^I$, $\mathfrak b_t$, $\bar c_t$
& Readiness order and exercise cost 0.125 each, direct cost one, release cost zero, and publication cost one. Period and cumulative budgets equal one half of the corresponding full action cost. No penalty replaces hard feasibility
...
& The benchmark uses $H_C=8$, $T_D=21$, $T_{\mathrm{clear}}=104$, and $\epsilon_{\mathrm{clear}}=10^{-6}$. Clearance runs with zero new demand, full route availability, and zero further action
...
Figure~\ref{fig:522-clearance} rules out nonclearance as the explanation for the
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/figure_5_2_2c_clearance_censoring.png}
    \caption{Clearance and censoring profiles}
    \label{fig:522-clearance}
\end{figure}
